...
internal class FakeTodoistApiClient : ITodoistApiClient
{
    ...
    public async Task<List<TodoistTask>> GetAllTasks()
    {
        ...
        {
            new TodoistTask("task id 1", "Buy milk (mock task) (10 min)", "Some description", 1, new List<Label>() { new Label("home"), new Label("pc") }),
            new TodoistTask("task id 2", "Buy carrots (mock task)", "Some description 2", 2, new List<Label>())
