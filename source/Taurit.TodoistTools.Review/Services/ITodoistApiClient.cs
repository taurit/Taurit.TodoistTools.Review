...
internal class FakeTodoistApiClient : ITodoistApiClient
{
    ...
    public async Task<List<TodoistTask>> GetAllTasks()
    {
        return new List<TodoistTask>
        {
            new TodoistTask("task id 1", "Buy milk (mock task)", "Some description", 0, new List<Label>(), 0),
            new TodoistTask("task id 2", "Buy carrots (mock task)", "Some description 2", 0, new List<Label>(), 0)
